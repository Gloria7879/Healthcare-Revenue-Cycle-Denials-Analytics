...
SELECT 
    ...
    -- Quantifies mandatory insurer discount write-offs
    (billed_amount - allowed_amount) AS contractual_adjustment,
    
    -- Tracks variance between allowed contract rate and cash recovered
    (allowed_amount - paid_amount) AS revenue_leakage,
    
    -- Flag for front-end workflow audit tracking
    CASE WHEN reason_code = 'Duplicate Claim' THEN 1 ELSE 0 END AS duplicate_denial,
    
    -- Measures true collection efficiency; NULLIF prevents division-by-zero crashes
...
SELECT 
    SUM(billed_amount) AS grand_total_billed,
    SUM(allowed_amount) AS grand_total_allowed,
    SUM(paid_amount) AS grand_total_collected,
    SUM(contractual_adjustment) AS grand_total_adjustments,
    SUM(revenue_leakage) AS grand_total_leakage,
    ROUND((SUM(paid_amount) / NULLIF(SUM(allowed_amount), 0)) * 100, 2) AS overall_rrr
FROM v_healthcare_revenue_integrity;